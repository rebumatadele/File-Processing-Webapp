...
interface AnthropicDataState {
  anthropic_usage: UsageResponse["ai_usage"]
  last_updated: string
}
...
export default function UsagePage() {
  ...
  const [anthropicData, setAnthropicData] = useState<AnthropicDataState | null>(null)
   ...
   */
    const fetchCacheData = async () => {
      ...
      try {
        // Now getCacheSize returns { cache_size: number }
        const sizeResponse = await getCacheSize()
        const contents: CacheContentsResponse = await listCacheContents()
        setCacheData({
          size: sizeResponse.cache_size,
          contents: contents.cache_contents,
        })
      } catch (error) {
  ...
  const fetchAnthropicData = async () => {
    ...
    try {
      // This returns the full UsageResponse: local_usage + anthropic_usage
      const usage: UsageResponse = await getAnthropicUsage()

      // We only store anthropic usage in state, but you could also store local_usage if needed
      setAnthropicData({
        anthropic_usage: usage.ai_usage,
  ...
  const fetchFileData = async () => {
    ...
    try {
      ...
      const size: number = await getUploadedFilesSize()
      setFileData({
        files,
        size: size || null, // If size is 0 or undefined, store as null
      })
    } catch (error) {
  ...
  const fetchProcessedData = async () => {
    ...
    try {
      ...
      const files = Object.entries(results).map(([filename, content]) => ({
        filename,
        content,
      }))
      const size: number = await getProcessedFilesSize()
      setProcessedData({
        files,
        size: size || null,
      })
